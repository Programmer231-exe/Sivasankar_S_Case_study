...
public class HMS_DAO {
	...
	public boolean registerAppointment(String username,String physicianId) {
		...
		PhysicianLoginDetails physicianloginDetails = (PhysicianLoginDetails)entityManager.createNamedQuery("getPhysicianLoginDetails").setParameter("physicianId",physicianId).getResultList().get(0);
	...
	@Transactional
	public List<PatientDiagnosisDetails> getDiagnosisReportofPatientforDoctor(
			String physicianId) {
		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		CriteriaQuery<PatientDiagnosisDetails> q = cb.createQuery(PatientDiagnosisDetails.class);
		Root<PatientDiagnosisDetails> root = q.from(PatientDiagnosisDetails.class);
